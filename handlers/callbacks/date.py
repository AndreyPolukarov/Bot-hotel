...
from keyboards.calendar.bot_calendar import CallbackData, Calendar
...
def input_date(call: CallbackQuery) -> None:
    ...
    if action == 'DAY':
        ...
        bot.set_state(call.message.chat.id, UserInfoState.input_date)
            ...
            if 'checkInDate' in data:
                ...
                if int(select_date) > checkin:
                    ...
                    if data['sort'] == 'DISTANCE':
                        bot.set_state(call.message.chat.id, UserInfoState.landmark_in)
                        bot.send_message(call.message.chat.id, 'Введите начало диапазона расстояния от центра '
                                                               '(от 0 миль).')
                    elif data['sort'] == 'PRICE_LOW_TO_HIGH':
                        logger.info('Переходит если была нажата на /low или /high')
                        proprties_v2_list.info_post_list(call.message, data)
                else:
                    bot.send_message(call.message.chat.id, 'Дата выезда должна быть больше даты заезда! '
                                                           'Повторите выбор даты!')
                    survey.my_calendar(call.message, 'выезда')
            else:
